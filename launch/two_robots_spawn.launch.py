...
def gen_robot_list(num_r):
    ...
    pose_array = [pose_1,pose_2]
    robot_name_array = ["rick","morty"]
...
def launch_setup(context, *args, **kwargs):
    ...
    launch_file_dir = os.path.join(get_package_share_directory(
        'barista_robot_description'), 'launch')
        ...
        spawn_robots_cmds.append(
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(os.path.join(
                    launch_file_dir, 'spawn_robot.launch.py')),
                launch_arguments={
...
def generate_launch_description():
    ...
    number_of_robots_arg = DeclareLaunchArgument(
        'number_of_robots', default_value='2')
